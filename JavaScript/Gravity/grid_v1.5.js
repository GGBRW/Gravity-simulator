...
canvas.addEventListener("mousedown", function(e) {
    ...
    if(e.which == 1) {
        if(e.shiftKey && grid.cells.length != 0) {
            for(var y2 = 0; y2 < Math.abs(Math.floor(y - grid.offset.y) - grid.cells[0].y) + 1; y2++) {
                for(var x2 = 0; x2 < Math.abs(Math.floor(x - grid.offset.x) - grid.cells[0].x) + 1; x2++) {
                    grid.createCell(grid.cells[0].x + x2,grid.cells[0].y + y2,grid.cellStates["selected"]);
                }
            }
        } else {
